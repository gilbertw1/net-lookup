...
fn download_maxmind_city_database(target_file: &Path) {
    ...
    reqwest::get("https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-City&license_key=Txrxy6vR98WaDFW0&suffix=tar.gz").unwrap().copy_to(&mut maxmind_archive_file).unwrap();
    Command::new("tar")
